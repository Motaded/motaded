...
/**
 * Import Arabic service content fields from reports/services_content_audit_ar.csv.
 *
 * Usage:
 *   ddev drush scr scripts/services_ar_import_from_audit_csv.php
 *   ddev drush scr scripts/services_ar_import_from_audit_csv.php -- --dry-run
 */
...
$csvPath = DRUPAL_ROOT . '/reports/services_content_audit_ar.csv';
$reportPath = DRUPAL_ROOT . '/reports/services_ar_import_report.csv';
...
/**
 * Build term label index for exact label matching in default + AR translation.
 *
...
$buildTermIndex = static function (string $vid) use ($termStorage, $normalize): array {
  ...
  foreach ($terms as $term) {
    ...
    $tid = (int) $term->id();
    $defaultLabel = $normalize((string) $term->label());
    if ($defaultLabel !== '') {
      $index[$defaultLabel] = $tid;
    }
    if ($term->hasTranslation('ar')) {
      $arLabel = $normalize((string) $term->getTranslation('ar')->label());
      if ($arLabel !== '') {
        $index[$arLabel] = $tid;
      }
    }
...
while (($row = fgetcsv($fh, 0, ',', '"', '\\')) !== FALSE) {
  ...
  if (!$node->hasTranslation('ar')) {
    $report[] = [(string) $nid, 'skip', 'no_ar_translation'];
    continue;
  }
  ...
  // Metatags (field_meta_tags stores JSON in "value").
