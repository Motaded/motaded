...
/**
 * @file
 * Export AR service translation content audit CSV.
 *
 * Scope:
 * - node type = page
 * - status = 1
 * - field_display_on_services = 1 (on source node)
 * - has ar translation
 *
 * Usage:
 *   drush scr scripts/services_content_audit_ar.php
 */

declare(strict_types=1);

$path = DRUPAL_ROOT . '/reports/services_content_audit_ar.csv';
$fp = fopen($path, 'w');
...
$yesNo = static function (bool $missing): string {
...
foreach ($nodes as $node) {
  ...
  foreach ($entity->get('field_taxonomy') as $item) {
    if ($item->entity) {
      $categories[] = $item->entity->label();
    }
  }
  $beneficiaries = [];
  foreach ($entity->get('field_beneficiaries') as $item) {
    if ($item->entity) {
      $beneficiaries[] = $item->entity->label();
    }
  }
  $tags = [];
